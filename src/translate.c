...
void init_filetrans_data(void)
{
    ...
    if (!GLOBALS->filesel_filter) {
        GLOBALS->filesel_filter = calloc_2(FILE_FILTER_MAX + 1, sizeof(char *));
    }
    if (!GLOBALS->xl_file_filter) {
        GLOBALS->xl_file_filter = calloc_2(FILE_FILTER_MAX + 1, sizeof(struct xl_tree_node *));
    }
...
{
    if (t->left)
        remove_file_filter_2(t->left);
    if (t->right)
        remove_file_filter_2(t->right);
    if (t->item)
        free_2(t->item);
    if (t->trans)
        free_2(t->trans);
...
static void load_file_filter(int which, const char *name)
{
    ...
    remove_file_filter(
        which,
        0); /* should never happen from GUI, but possible from save files or other weirdness */
    ...
    while (!feof(f)) {
        ...
        if (s) {
            ...
            while (*lhs && isspace((int)(unsigned char)*lhs))
                lhs++;
            if (lhs) {
                char *rhs = lhs;
                ...
                {
                    while (*rhs && !isspace((int)(unsigned char)*rhs))
                        rhs++;
                    if (*rhs) {
                        char *xlt = rhs + 1;
                        *rhs = 0;

                        while (*xlt && isspace((int)(unsigned char)*xlt))
                            xlt++;
                        if (*xlt) {
                            GLOBALS->xl_file_filter[which] =
                                xl_insert(lhs, GLOBALS->xl_file_filter[which], xlt);
                        }
                    }
                }
            }
...
static void add_filter_callback_2(GtkWidget *widget, GtkWidget *nothing)
{
    ...
    if (GLOBALS->xl_file_filter[GLOBALS->num_file_filters] &&
        ...
        gtk_list_store_set(GTK_LIST_STORE(GLOBALS->sig_store_translate),
                           &iter,
                           NAME_COLUMN,
                           GLOBALS->filesel_filter[GLOBALS->num_file_filters],
                           -1);
...
void trans_searchbox(const char *title)
{
    ...
    GLOBALS->window_translate_c_11 =
        gtk_window_new(GLOBALS->disable_window_manager ? GTK_WINDOW_POPUP : GTK_WINDOW_TOPLEVEL);
    install_focus_cb(GLOBALS->window_translate_c_11,
                     ((char *)&GLOBALS->window_translate_c_11) - ((char *)GLOBALS));

    gtk_window_set_title(GTK_WINDOW(GLOBALS->window_translate_c_11), title);
    gtkwave_signal_connect(GLOBALS->window_translate_c_11,
                           "delete_event",
                           (GCallback)destroy_callback,
                           NULL);
    gtk_container_set_border_width(GTK_CONTAINER(GLOBALS->window_translate_c_11), 12);

    GtkWidget *main_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
    gtk_container_add(GTK_CONTAINER(GLOBALS->window_translate_c_11), main_vbox);

    GLOBALS->sig_store_translate = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING);
    GtkWidget *sig_view =
        gtk_tree_view_new_with_model(GTK_TREE_MODEL(GLOBALS->sig_store_translate));
    ...
    gtk_tree_selection_set_select_function(GLOBALS->sig_selection_translate,
    ...
    GtkWidget *add_button = gtk_button_new_with_label("Add Filter to List");
    ...
    gtk_widget_show_all(GLOBALS->window_translate_c_11);
}
...
static void set_current_translate_generic(const char *name, int typ)
{
    ...
    if (typ) {
        for (i = 1; i < GLOBALS->num_file_filters + 1; i++) {
            if (!strcmp(GLOBALS->filesel_filter[i], name)) {
                GLOBALS->current_translate_file = i;
                return;
            }
        }
