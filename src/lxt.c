...
{
    return (
        (unsigned int)(*((unsigned short *)(((unsigned char *)(GLOBALS->mm_lxt_c_1)) + (offset)))));
}
...
static TimeType bsearch_mvl_timechain(int key)
{
    ...
    if (bsearch((void *)&key,
                (void *)GLOBALS->positional_information_lxt_c_1,
...
static void build_dict(void)
{
...
#if defined __MINGW32__
    {
        ...
        tmp = tmpfile();
        if (!tmp) {
            fprintf(stderr, LXTHDR "could not open decompression tempfile, exiting.\n");
            exit(255);
        }
        fwrite(t, GLOBALS->zdictionary_predec_size_lxt_c_1, 1, tmp);
        fseek(tmp, 0, SEEK_SET);
        zhandle = gzdopen(dup(fileno(tmp)), "rb");
    }
#else
    if (offs != lseek(GLOBALS->fd_lxt_c_1, offs, SEEK_SET)) {
        fprintf(stderr, LXTHDR "dict lseek error at offset %08x\n", (unsigned int)offs);
        exit(255);
    }
    ...
    if (rc != total_mem) {
        fprintf(stderr, LXTHDR "decompression size disparity  %d bytes (vs %d)\n", rc, total_mem);
        exit(255);
    }

    GLOBALS->dict_string_mem_array_lxt_c_1 =
        (char **)calloc_2(GLOBALS->dict_num_entries_lxt_c_1, sizeof(char *));
    pnt = decmem;
    ...
    fprintf(stderr,
            LXTHDR "...expanded %d entries from %08x into %08x bytes.\n",
            GLOBALS->dict_num_entries_lxt_c_1,
            GLOBALS->zdictionary_predec_size_lxt_c_1,
            GLOBALS->dict_string_mem_required_lxt_c_1);
}
...
static void build_facs(char *fname, char **f_name, struct Node *node_block)
{
    ...
    if (GLOBALS->zfacname_size_lxt_c_1) {
        ...
        fwrite(t, GLOBALS->zfacname_size_lxt_c_1, 1, tmp);
        fseek(tmp, 0, SEEK_SET);
        zhandle = gzdopen(dup(fileno(tmp)), "rb");
#else
        if (offs != lseek(GLOBALS->fd_lxt_c_1, offs, SEEK_SET)) {
            fprintf(stderr, LXTHDR "zfacname lseek error at offset %08x\n", (unsigned int)offs);
            exit(255);
        }
        zhandle = gzdopen(dup(GLOBALS->fd_lxt_c_1), "rb");
#endif
        ...
        decmem = malloc_2(total_mem = GLOBALS->zfacname_predec_size_lxt_c_1);
        GLOBALS->mm_lxt_c_1 = decmem;

        rc = gzread(zhandle, decmem, total_mem);
        DEBUG(printf(LXTHDR "section offs for name decompression = %08x of len %d\n",
                     offs,
                     GLOBALS->zfacname_size_lxt_c_1));
        DEBUG(printf(LXTHDR "Decompressed size is %d bytes (vs %d)\n", rc, total_mem));
        if (rc != total_mem) {
            fprintf(stderr,
                    LXTHDR "decompression size disparity  %d bytes (vs %d)\n",
                    rc,
                    total_mem);
            exit(255);
        }
    ...
    if (GLOBALS->zfacgeometry_size_lxt_c_1) {
        ...
        fwrite(t, GLOBALS->zfacgeometry_size_lxt_c_1, 1, tmp);
        fseek(tmp, 0, SEEK_SET);
        zhandle = gzdopen(dup(fileno(tmp)), "rb");
#else
        if (offs != lseek(GLOBALS->fd_lxt_c_1, offs, SEEK_SET)) {
            fprintf(stderr, LXTHDR "zfacgeometry lseek error at offset %08x\n", (unsigned int)offs);
            exit(255);
        }
        zhandle = gzdopen(dup(GLOBALS->fd_lxt_c_1), "rb");
#endif
    ...
    for (i = 0; i < GLOBALS->numfacs; i++) {
        ...
        GLOBALS->mvlfacs_lxt_c_2[i].len = (node_block[i].lsi > node_block[i].msi)
                                              ? (node_block[i].lsi - node_block[i].msi + 1)
                                              : (node_block[i].msi - node_block[i].lsi + 1);

        if (GLOBALS->mvlfacs_lxt_c_2[i].len > GLOBALS->lt_len_lxt_c_1)
            GLOBALS->lt_len_lxt_c_1 = GLOBALS->mvlfacs_lxt_c_2[i].len;
        DEBUG(printf(LXTHDR "%s[%d:%d]\n", f_name[i], node_block[i].msi, node_block[i].lsi));

        offs += 0x10;
    }

    GLOBALS->lt_buf_lxt_c_1 =
        malloc_2(GLOBALS->lt_len_lxt_c_1 > 255
                     ? GLOBALS->lt_len_lxt_c_1
                     : 256); /* in order to keep trivial LXT files from overflowing their buffer */
...
static void build_facs2(char *fname)
{
    ...
    if (GLOBALS->time_table_offset_lxt_c_1) {
        ...
        DEBUG(
            printf(LXTHDR "Time table position: %08x\n", GLOBALS->time_table_offset_lxt_c_1 + 12));
        ...
        if (GLOBALS->ztime_table_size_lxt_c_1) {
            ...
            fwrite(t, GLOBALS->ztime_table_size_lxt_c_1, 1, tmp);
            fseek(tmp, 0, SEEK_SET);
            zhandle = gzdopen(dup(fileno(tmp)), "rb");
#else
            if ((offs + 4) != lseek(GLOBALS->fd_lxt_c_1, offs + 4, SEEK_SET)) {
                fprintf(stderr,
                        LXTHDR "ztime_table lseek error at offset %08x\n",
                        (unsigned int)offs);
                exit(255);
            }
            zhandle = gzdopen(dup(GLOBALS->fd_lxt_c_1), "rb");
            ...
            GLOBALS->mmcache_lxt_c_1 = GLOBALS->mm_lxt_c_1;
            total_mem =
                4 + 4 + (GLOBALS->total_cycles_lxt_c_2 * 4) + (GLOBALS->total_cycles_lxt_c_2 * 4);
            decmem = malloc_2(total_mem);
            GLOBALS->mm_lxt_c_1 = decmem;
            ...
            DEBUG(printf(LXTHDR "section offs for timetable decompression = %08x of len %d\n",
                         offs,
                         GLOBALS->ztime_table_size_lxt_c_1));
            DEBUG(printf(LXTHDR "Decompressed size is %d bytes (vs %d)\n", rc, total_mem));
            if (rc != total_mem) {
                fprintf(stderr,
                        LXTHDR "decompression size disparity  %d bytes (vs %d)\n",
        ...
        GLOBALS->positional_information_lxt_c_1 =
            (int *)malloc_2(GLOBALS->total_cycles_lxt_c_2 * sizeof(int));
        last_position = 0;
        offs += 8;
        for (i = 0; i < GLOBALS->total_cycles_lxt_c_2; i++) {
            last_position = GLOBALS->positional_information_lxt_c_1[i] =
                get_32(offs) + last_position;
            offs += 4;
        }
    ...
    } else /* 64-bit read */
    {
        ...
        if (GLOBALS->ztime_table_size_lxt_c_1) {
            ...
            GLOBALS->mmcache_lxt_c_1 = GLOBALS->mm_lxt_c_1;
            total_mem =
                8 + 8 + (GLOBALS->total_cycles_lxt_c_2 * 4) + (GLOBALS->total_cycles_lxt_c_2 * 8);
            decmem = malloc_2(total_mem);
            GLOBALS->mm_lxt_c_1 = decmem;
    ...
    if (GLOBALS->sync_table_offset_lxt_c_1) {
        ...
        if (GLOBALS->zsync_table_size_lxt_c_1) {
            ...
            fwrite(t, GLOBALS->zsync_table_size_lxt_c_1, 1, tmp);
            fseek(tmp, 0, SEEK_SET);
            zhandle = gzdopen(dup(fileno(tmp)), "rb");
#else
            if (offs != lseek(GLOBALS->fd_lxt_c_1, offs, SEEK_SET)) {
                fprintf(stderr,
                        LXTHDR "zsync_table lseek error at offset %08x\n",
                        (unsigned int)offs);
                exit(255);
            }
            zhandle = gzdopen(dup(GLOBALS->fd_lxt_c_1), "rb");
#endif
            GLOBALS->mmcache_lxt_c_1 = GLOBALS->mm_lxt_c_1;
            decmem = malloc_2(total_mem = GLOBALS->numfacs * 4);
            GLOBALS->mm_lxt_c_1 = decmem;

            rc = gzread(zhandle, decmem, total_mem);
            DEBUG(printf(LXTHDR "section offs for synctable decompression = %08x of len %d\n",
                         offs,
                         GLOBALS->zsync_table_size_lxt_c_1));
            DEBUG(printf(LXTHDR "Decompressed size is %d bytes (vs %d)\n", rc, total_mem));
            if (rc != total_mem) {
                fprintf(stderr,
                        LXTHDR "decompression size disparity  %d bytes (vs %d)\n",
    ...
    if (GLOBALS->zchg_size_lxt_c_1) {
        ...
        if (GLOBALS->zchg_predec_size_lxt_c_1 > LXT_MMAP_MALLOC_BOUNDARY) {
            ...
            if (GLOBALS->change_field_offset_lxt_c_1 !=
                lseek(GLOBALS->fd_lxt_c_1, GLOBALS->change_field_offset_lxt_c_1, SEEK_SET)) {
                fprintf(stderr,
                        LXTHDR "lseek error at offset %08x\n",
                        (unsigned int)GLOBALS->change_field_offset_lxt_c_1);
                exit(255);
            }

            is_bz2 = (read(GLOBALS->fd_lxt_c_1, &testbyte, 2)) && (testbyte[0] == 'B') &&
                     (testbyte[1] == 'Z');
            ...
            if (is_bz2) {
                ...
                while (len) {
                    ...
                    if (rc != siz) {
                        fprintf(stderr,
                                LXTHDR "gzread error to tempfile %d (act) vs %d (exp), exiting.\n",
                                rc,
                                siz);
                        exit(255);
                    }
                    if (1 != fwrite(buf, siz, 1, tmp)) {
                        fprintf(stderr, LXTHDR "fwrite error to tempfile, exiting.\n");
                        exit(255);
                    };
                    len -= siz;
                }
                ...
                fprintf(stderr,
                        LXTHDR "...expanded %08x into %08x bytes.\n",
            ...
            GLOBALS->mm_lxt_c_1 =
                (void *)((char *)GLOBALS->mm_lxt_c_1 -
                         4); /* because header and version don't exist in packed change records */
        } else
#endif
        {
            unsigned int len = GLOBALS->zchg_predec_size_lxt_c_1;
            ...
            {
                unsigned char *t =
                    (char *)GLOBALS->mm_lxt_c_1 + GLOBALS->change_field_offset_lxt_c_1;
                tmp = tmpfile();
                if (!tmp) {
                    fprintf(stderr, LXTHDR "could not open decompression tempfile, exiting.\n");
                    exit(255);
                }
                fwrite(t, GLOBALS->zchg_size_lxt_c_1, 1, tmp);
                fseek(tmp, 0, SEEK_SET);
                is_bz2 = (get_byte(GLOBALS->change_field_offset_lxt_c_1) == 'B') &&
                         (get_byte(GLOBALS->change_field_offset_lxt_c_1 + 1) == 'Z');
            }
#else
            if (GLOBALS->change_field_offset_lxt_c_1 !=
                lseek(GLOBALS->fd_lxt_c_1, GLOBALS->change_field_offset_lxt_c_1, SEEK_SET)) {
            ...
            if (is_bz2) {
                ...
                while (len) {
                    ...
                    if (rc != siz) {
                        fprintf(stderr,
                                LXTHDR
                                "BZ2_bzread error to buffer %d (act) vs %d (exp), exiting.\n",
                                rc,
                                siz);
                        exit(255);
                    }
                ...
                while (len) {
                    ...
                    if (rc != siz) {
                        fprintf(stderr,
                                LXTHDR "gzread error to buffer %d (act) vs %d (exp), exiting.\n",
                                rc,
                                siz);
                        exit(255);
                    }
    ...
    if (!GLOBALS->sync_table_offset_lxt_c_1) {
        ...
        fclose(tmp);
        fprintf(stderr,
                LXTHDR "%d linear records converted into %08x bytes.\n",
                num_records,
                GLOBALS->fpos_lxt_c_1 - 4);

#if !defined __MINGW32__
        if (GLOBALS->zchg_predec_size_lxt_c_1 > LXT_MMAP_MALLOC_BOUNDARY) {
            munmap((char *)GLOBALS->mm_lxt_c_1 + 4, GLOBALS->zchg_predec_size_lxt_c_1);
            close(GLOBALS->fd_lxt_c_1);
...
#endif
        GLOBALS->mm_lxt_c_1 =
            mmap(NULL, GLOBALS->fpos_lxt_c_1 - 4, PROT_READ, MAP_SHARED, recfd, 0);
        GLOBALS->mm_lxt_mmap_addr = GLOBALS->mm_lxt_c_1;
    ...
    switch (v2) {
        case 0x00: /* MVL2 */
        {
            ...
            if ((GLOBALS->dict_string_mem_array_lxt_c_1) && (l > GLOBALS->dict_width_lxt_c_1)) {
                ...
                if ((!GLOBALS->dict_16_offset_lxt_c_1) ||
                    (offs < GLOBALS->dict_16_offset_lxt_c_1)) {
                    dictpos = get_byte(offs);
                } else if ((!GLOBALS->dict_24_offset_lxt_c_1) ||
                           (offs < GLOBALS->dict_24_offset_lxt_c_1)) {
                    dictpos = get_16(offs);
                } else if ((!GLOBALS->dict_32_offset_lxt_c_1) ||
                           (offs < GLOBALS->dict_32_offset_lxt_c_1)) {
                    dictpos = get_24(offs);
                } else {
                    dictpos = get_32(offs);
                }
                ...
                if (dictpos <= GLOBALS->dict_num_entries_lxt_c_1) {
                    ...
                    *(pnt++) = '1';
                    ...
                    fprintf(stderr,
                            LXTHDR "dict entry at offset %08x [%d] out of range, ignoring!\n",
                            dictpos,
                            (unsigned int)offs);
                    for (j = 0; j < l; j++) {
                        *(pnt++) = '0';
...
TimeType lxt_main(char *fname)
{
...
#endif
    GLOBALS->mm_lxt_c_1 =
        mmap(NULL, GLOBALS->f_len_lxt_c_1, PROT_READ, MAP_SHARED, GLOBALS->fd_lxt_c_1, 0);
    GLOBALS->mm_lxt_mmap_addr = GLOBALS->mm_lxt_c_1;
    ...
    if ((GLOBALS->version_lxt_c_1 = get_16((off_t)2)) > LT_VERSION) {
        fprintf(stderr,
                "Version %d of LXT format AETs not supported, exiting.\n",
    ...
    while ((tag = get_byte(tagpnt)) != LT_SECTION_END) {
        ...
        switch (tag) {
            case LT_SECTION_CHG:
                GLOBALS->change_field_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_CHG at %08x\n", offset));
                break;
            case LT_SECTION_SYNC_TABLE:
                GLOBALS->sync_table_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_SYNC_TABLE at %08x\n", offset));
                break;
            case LT_SECTION_FACNAME:
                GLOBALS->facname_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_FACNAME at %08x\n", offset));
                break;
            case LT_SECTION_FACNAME_GEOMETRY:
                GLOBALS->facgeometry_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_FACNAME_GEOMETRY at %08x\n", offset));
                break;
            case LT_SECTION_TIMESCALE:
                GLOBALS->timescale_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_TIMESCALE at %08x\n", offset));
                break;
            case LT_SECTION_TIME_TABLE:
                GLOBALS->time_table_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_TIME_TABLE at %08x\n", offset));
                break;
            case LT_SECTION_TIME_TABLE64:
                GLOBALS->time_table_offset64_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_TIME_TABLE64 at %08x\n", offset));
                break;
            case LT_SECTION_INITIAL_VALUE:
                GLOBALS->initial_value_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_INITIAL_VALUE at %08x\n", offset));
                break;
            case LT_SECTION_DOUBLE_TEST:
                GLOBALS->double_test_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_DOUBLE_TEST at %08x\n", offset));
                break;

            case LT_SECTION_ZFACNAME_PREDEC_SIZE:
                GLOBALS->zfacname_predec_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZFACNAME_PREDEC_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZFACNAME_SIZE:
                GLOBALS->zfacname_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZFACNAME_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZFACNAME_GEOMETRY_SIZE:
                GLOBALS->zfacgeometry_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZFACNAME_GEOMETRY_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZSYNC_SIZE:
                GLOBALS->zsync_table_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZSYNC_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZTIME_TABLE_SIZE:
                GLOBALS->ztime_table_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZTIME_TABLE_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZCHG_PREDEC_SIZE:
                GLOBALS->zchg_predec_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZCHG_PREDEC_SIZE = %08x\n", offset));
                break;
            case LT_SECTION_ZCHG_SIZE:
                GLOBALS->zchg_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZCHG_SIZE = %08x\n", offset));
                break;

            case LT_SECTION_ZDICTIONARY:
                GLOBALS->zdictionary_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZDICTIONARY = %08x\n", offset));
                break;
            case LT_SECTION_ZDICTIONARY_SIZE:
                GLOBALS->zdictionary_predec_size_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_ZDICTIONARY_SIZE = %08x\n", offset));
                break;

            case LT_SECTION_EXCLUDE_TABLE:
                GLOBALS->exclude_offset_lxt_c_1 = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_EXCLUDE_TABLE = %08x\n", offset));
                break;

            case LT_SECTION_TIMEZERO:
                GLOBALS->lxt_timezero_offset = offset;
                DEBUG(printf(LXTHDR "LT_SECTION_TIMEZERO = %08x\n", offset));
                break;

            default:
                fprintf(stderr, "Skipping unknown section tag %02x.\n", tag);
                break;
        }
    }
    ...
    GLOBALS->resolve_lxt_alias_to =
        (struct Node **)calloc_2(GLOBALS->numfacs, sizeof(struct Node *));
    ...
    if (GLOBALS->initial_value_offset_lxt_c_1) {
        switch (get_byte(GLOBALS->initial_value_offset_lxt_c_1)) {
            case 0:
                GLOBALS->initial_value_lxt_c_1 = AN_0;
                break;
            case 1:
                GLOBALS->initial_value_lxt_c_1 = AN_1;
                break;
            case 2:
                GLOBALS->initial_value_lxt_c_1 = AN_Z;
                break;
            case 4:
                GLOBALS->initial_value_lxt_c_1 = AN_H;
                break;
            case 5:
                GLOBALS->initial_value_lxt_c_1 = AN_U;
                break;
            case 6:
                GLOBALS->initial_value_lxt_c_1 = AN_W;
                break;
            case 7:
                GLOBALS->initial_value_lxt_c_1 = AN_L;
                break;
            case 8:
                GLOBALS->initial_value_lxt_c_1 = AN_DASH;
                break;
            default:
    ...
    if (GLOBALS->zdictionary_offset_lxt_c_1) {
        if (GLOBALS->zdictionary_predec_size_lxt_c_1) {
            ...
            fprintf(stderr,
                    "LXT '%s' is missing a zdictionary_predec_size chunk, exiting.\n",
                    fname);
            vcd_exit(255);
        }
    }
    ...
    for (i = 0; i < GLOBALS->numfacs; i++) {
        ...
        if ((f->len > 1) &&
            (!(f->flags & (LT_SYM_F_INTEGER | LT_SYM_F_DOUBLE | LT_SYM_F_STRING)))) {
            ...
            if (gatecmp) {
                ...
                symadd_name_exists_sym_exists(s, str, 0);
                if ((prevsym) && (revcmp) &&
                    (!strchr(f_name[i], '\\'))) /* allow chaining for search functions.. */
                {
...
void import_lxt_trace(nptr np)
{
    ...
    if (np->mv.mvlfac->flags & LT_SYM_F_ALIAS) {
        ...
        if (GLOBALS->resolve_lxt_alias_to[alias]) {
            if (!GLOBALS->resolve_lxt_alias_to[np->mv.mvlfac - GLOBALS->mvlfacs_lxt_c_2])
                GLOBALS->resolve_lxt_alias_to[np->mv.mvlfac - GLOBALS->mvlfacs_lxt_c_2] =
                    GLOBALS->resolve_lxt_alias_to[alias];
        } else {
        ...
        while (f->flags & LT_SYM_F_ALIAS) {
            ...
            if (GLOBALS->resolve_lxt_alias_to[f->node_alias]) {
                if (!GLOBALS->resolve_lxt_alias_to[np->mv.mvlfac - GLOBALS->mvlfacs_lxt_c_2])
                    GLOBALS->resolve_lxt_alias_to[np->mv.mvlfac - GLOBALS->mvlfacs_lxt_c_2] =
                        GLOBALS->resolve_lxt_alias_to[f->node_alias];
            } else {
    ...
    if (GLOBALS->resolve_lxt_alias_to[np->mv.mvlfac -
                                      GLOBALS->mvlfacs_lxt_c_2]) /* in case we're an alias head for
                                                                    later.. */
    {
    ...
    if (f->flags & (LT_SYM_F_DOUBLE | LT_SYM_F_STRING)) {
        ...
        if (f->flags & LT_SYM_F_STRING)
            histent_head->flags |= HIST_STRING;
    } else {
        if (len > 1) {
            histent_head->v.h_vector = (char *)malloc_2(len);
            for (i = 0; i < len; i++)
                histent_head->v.h_vector[i] = AN_X;
        } else {
        ...
        while (offs) {
            ...
            if ((w = ((v = get_byte(offs)) & 0xF)) > 0xb) {
                ...
                switch (get_byte(offsminus1) & 0xF0) {
                    ...
                    default:
                        fprintf(stderr,
                                "Unknown %02x at offset: %08x\n",
                                get_byte(offsminus1),
                                (unsigned int)offsminus1);
                        exit(0);
                }
                offsminus2 = offsminus1 - offsdelta - 2;

                delta = (time_offsminus1 = bsearch_mvl_timechain(offsminus1)) -
                        bsearch_mvl_timechain(offsminus2);
                ...
                if (len > 1) {
                    ...
                    if (!GLOBALS->lxt_clock_compress_to_z) {
                        ...
                        if ((vval != 0) && (vval != 3) && (vval != 4)) {
                            fprintf(stderr,
                                    "Unexpected clk compress byte %02x at offset: %08x\n",
                                    get_byte(offsminus1),
                                    (unsigned int)offsminus1);
                            exit(0);
                        }
                        ...
                        switch (w & 3) {
                            case 0:
                                reps = get_byte(offs + skip);
                                break;
                            case 1:
                                reps = get_16(offs + skip);
                                break;
                            case 2:
                                reps = get_24(offs + skip);
                                break;
                            case 3:
                                reps = get_32(offs + skip);
                                break;
                ...
                offs = offsminus1; /* no need to recalc it again! */
                continue;
            } else if ((tmval = bsearch_mvl_timechain(offs)) !=
                       prevtmval) /* get rid of glitches (if even possible) */
            {
                ...
                if (!(f->flags & (LT_SYM_F_DOUBLE | LT_SYM_F_STRING))) {
                ...
                } else if (f->flags & LT_SYM_F_DOUBLE) {
                    ...
                    if (GLOBALS->double_is_native_lxt_c_1) {
                        ...
                        memcpy(&htemp->v.h_double,
                               ((char *)GLOBALS->mm_lxt_c_1 + offs_dbl),
                               sizeof(double));
#else
                    ...
                    histent_head = htemp;
                    np->numhist++;
                } else { /* defaults to if(f->flags&LT_SYM_F_STRING) */
