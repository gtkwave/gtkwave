    ...
    if (!GLOBALS->quiet_checkmenu) /* when gtkwave_mlist_t check menuitems are being initialized */
    {
        ...
        if (GLOBALS->text_status_c_2) {
            gtk_text_buffer_insert(
                gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
                &GLOBALS->iter_status_c_3,
                str,
                -1);
            ...
            GtkTextMark *mark = gtk_text_buffer_get_mark(
                gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
                "end");
            gtk_text_view_scroll_mark_onscreen(GTK_TEXT_VIEW(GLOBALS->text_status_c_2),
                                               GTK_TEXT_MARK(mark));
        } else {
...
GtkWidget *create_text(void)
{
    ...
    gtk_text_buffer_get_start_iter(
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
        &GLOBALS->iter_status_c_3);
    GLOBALS->bold_tag_status_c_3 = gtk_text_buffer_create_tag(
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
        "bold",
        "weight",
        PANGO_WEIGHT_BOLD,
        NULL);
    gtk_text_buffer_get_end_iter(gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
                                 &iter);
    gtk_text_buffer_create_mark(gtk_text_view_get_buffer(GTK_TEXT_VIEW(GLOBALS->text_status_c_2)),
                                "end",
                                &iter,
                                FALSE);

    gtk_container_add(GTK_CONTAINER(sw), GLOBALS->text_status_c_2);
