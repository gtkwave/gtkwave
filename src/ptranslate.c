...
void init_proctrans_data(void)
{
    ...
    if (!GLOBALS->procsel_filter) {
        GLOBALS->procsel_filter = calloc_2(PROC_FILTER_MAX + 1, sizeof(char *));
    }
    if (!GLOBALS->proc_filter) {
        GLOBALS->proc_filter = calloc_2(PROC_FILTER_MAX + 1, sizeof(struct pipe_ctx *));
    }
...
static void load_proc_filter(int which, char *name)
{
    ...
    /* remove_proc_filter(which, 0); ... should never happen from GUI, but perhaps possible from
...
static void add_filter_callback_2(GtkWidget *widget, GtkWidget *nothing)
{
    ...
    if (GLOBALS->proc_filter[GLOBALS->num_proc_filters]) {
        if (GLOBALS->procsel_filter[GLOBALS->num_proc_filters])
            free_2(GLOBALS->procsel_filter[GLOBALS->num_proc_filters]);
        GLOBALS->procsel_filter[GLOBALS->num_proc_filters] =
        ...
        gtk_list_store_set(GTK_LIST_STORE(GLOBALS->sig_store_ptranslate),
                           &iter,
                           NAME_COLUMN,
                           GLOBALS->procsel_filter[GLOBALS->num_proc_filters],
                           -1);
...
static void add_filter_callback(GtkWidget *widget, GtkWidget *nothing)
{
    ...
    fileselbox("Select Filter Process",
...
void ptrans_searchbox(const char *title)
{
    ...
    GLOBALS->window_ptranslate_c_5 =
        gtk_window_new(GLOBALS->disable_window_manager ? GTK_WINDOW_POPUP : GTK_WINDOW_TOPLEVEL);
    install_focus_cb(GLOBALS->window_ptranslate_c_5,
                     ((char *)&GLOBALS->window_ptranslate_c_5) - ((char *)GLOBALS));

    gtk_window_set_title(GTK_WINDOW(GLOBALS->window_ptranslate_c_5), title);
    gtkwave_signal_connect(GLOBALS->window_ptranslate_c_5,
                           "delete_event",
                           (GCallback)destroy_callback,
                           NULL);
    gtk_container_set_border_width(GTK_CONTAINER(GLOBALS->window_ptranslate_c_5), 12);

    GtkWidget *main_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
    gtk_container_add(GTK_CONTAINER(GLOBALS->window_ptranslate_c_5), main_vbox);

    GLOBALS->sig_store_ptranslate = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING);
    GtkWidget *sig_view =
        gtk_tree_view_new_with_model(GTK_TREE_MODEL(GLOBALS->sig_store_ptranslate));
    ...
    gtk_tree_selection_set_select_function(GLOBALS->sig_selection_ptranslate,
    ...
    GtkWidget *add_button = gtk_button_new_with_label("Add Proc Filter to List");
    ...
    gtk_widget_show_all(GLOBALS->window_ptranslate_c_5);
}
...
void set_current_translate_proc(char *name)
{
    ...
    for (i = 1; i < GLOBALS->num_proc_filters + 1; i++) {
        if (!strcmp(GLOBALS->procsel_filter[i], name)) {
            GLOBALS->current_translate_proc = i;
            return;
        }
    }
