...
const char *ae2_loader_fail_msg =
    "Sorry, AET2 support was not compiled into this executable, exiting.\n\n";

GwTime ae2_main(char *fname, char *skip_start, char *skip_end)
{
    (void)fname;
    (void)skip_start;
    (void)skip_end;
...
static void *adb_alloc_2(size_t siz)
{
    if (GLOBALS->adb_alloc_pool_base) {
        if ((siz + GLOBALS->adb_alloc_idx) <= WAVE_ADB_ALLOC_POOL_SIZE) {
            ...
            return ((void *)m);
        } else if (siz >= WAVE_ADB_ALLOC_ALTREQ_SIZE) {
...
static void ae2_read_value_2a(AE2_HANDLE handle,
                              unsigned long idx,
                              uint64_t cycle,
                              char *value,
                              int tot_length)
    ...
    if (numTerms) {
        for (i = 0; i < numTerms; i++) {
            ...
            if (fr2.s) {
                ...
                memset(value + offs, 'Z', fr2.length); /* should never happen except when alias is
                                                          superset of available model facs */
            }
            offs += length;
        }
    } else {
...
static uint64_t ae2_read_next_value_2a(AE2_HANDLE handle,
                                       unsigned long idx,
                                       uint64_t cycle,
                                       char *value,
                                       int tot_length)
{
    ...
    if (numTerms) {
        for (i = 0; i < numTerms; i++) {
            ...
            if (fr2.s) {
                t_cyc = ae2_read_next_value(
                    handle,
                    &fr2,
                    cycle,
                    value + offs); /* simply want to calculate next value change time */
            } else {
                /* should never happen except when alias is superset of available model facs */
                t_cyc = cycle;
            }
        ...
        ae2_read_value_2a(handle,
                          idx + 1,
                          cyc,
                          value,
                          tot_length); /* reread at that calculated value change time */
    } else {
        /* should never happen except when alias is superset of available model facs */
        memset(value, 'Z', tot_length);
        value[tot_length] = 0;
...
static uint64_t ae2_read_next_value_2(AE2_HANDLE handle,
                                      AE2_FACREF *fr,
                                      uint64_t cycle,
                                      char *value)
{
...
GwTime ae2_main(char *fname, char *skip_start, char *skip_end)
{
    ...
    struct Node *n;
    struct symbol *s;
    GwTime first_cycle, last_cycle /* , total_cycles */; /* scan-build */
    int total_rows = 0;
    ...
    if ((!(GLOBALS->ae2_f = fopen(fname, "rb"))) ||
        (!(GLOBALS->ae2 = ae2_read_initialize(GLOBALS->ae2_f)))) {
        if (GLOBALS->ae2_f) {
            fclose(GLOBALS->ae2_f);
            GLOBALS->ae2_f = NULL;
        }

        return (GW_TIME_CONSTANT(0)); /* look at GLOBALS->ae2 in caller for success status... */
    }
    ...
    if (kw) {
        ...
        GLOBALS->adb = adb_open_embed(GLOBALS->adb_alias_stream_file,
                                      NULL,
                                      alloc_fn,
                                      free_fn,
                                      adb_msg_fn,
        ...
        if (GLOBALS->adb) {
            ...
            fn = adb_map_ids(GLOBALS->adb,
                             symbol_fn,
                             GLOBALS->ae2); /* iteratively replaces all .id with FACIDX */

            fprintf(stderr,
                    AET2_RDLOAD "Encountered %lu aliases referencing %lu facs.\n",
                    GLOBALS->ae2_num_aliases,
                    fn);
        }
    }
#endif
    ...
    for (i = 0; i < GLOBALS->ae2_num_facs; i++) {
        ...
        if (GLOBALS->ae2_fr[match_idx].row > AE2_MAX_ROWS) {
            ...
            fprintf(stderr,
                    AET2_RDLOAD "Warning: Reduced array %s to %d rows.\n",
                    buf,
                    AE2_MAX_ROWS);
        }
        total_rows += (GLOBALS->ae2_fr[match_idx].row > 0) ? GLOBALS->ae2_fr[match_idx].row : 1;
        if (GLOBALS->ae2_fr[match_idx].row == 1)
    ...
    for (i = 0; i < GLOBALS->ae2_num_aliases; i++) {
        ...
        if ((numTerms = adb_load_alias_def(GLOBALS->adb, idx, GLOBALS->adb_terms))) {
            if (GLOBALS->adb_terms[0].first > GLOBALS->adb_terms[0].last) {
                GLOBALS->ae2_fr[match_idx].length =
                    GLOBALS->adb_terms[0].first - GLOBALS->adb_terms[0].last + 1;
            } else {
                GLOBALS->ae2_fr[match_idx].length =
                    GLOBALS->adb_terms[0].last - GLOBALS->adb_terms[0].first + 1;
            }

            GLOBALS->adb_idx_first[i] = GLOBALS->adb_terms[0].first;
            GLOBALS->adb_idx_last[i] = GLOBALS->adb_terms[0].last;

            GLOBALS->ae2_fr[match_idx].s =
                idx + GLOBALS->ae2_num_facs; /* bias aliases after regular facs */
    ...
    first_cycle = (GwTime)ae2_read_start_cycle(GLOBALS->ae2);
    last_cycle = (GwTime)ae2_read_end_cycle(GLOBALS->ae2);
    ...
    for (i = 0; i < (unsigned int)GLOBALS->numfacs; i++) {
        ...
        if (GLOBALS->ae2_fr[match_idx].length > 1) {
...
#endif
            {
                ...
                len2 = sprintf_2_2d(buf + len,
                                    GLOBALS->adb_idx_first[i - GLOBALS->ae2_num_facs],
                                    GLOBALS->adb_idx_last[i - GLOBALS->ae2_num_facs]);
            }
#endif
            ...
            if (clen > GLOBALS->longestname)
                GLOBALS->longestname = clen;
        ...
        for (row_iter = 0; row_iter < mx_row; row_iter++) {
            ...
            n[row_iter].mv.mvlfac =
                (struct fac *)(GLOBALS->ae2_fr + match_idx); /* to keep from having to allocate
                                                                duplicate mvlfac struct */
...
#ifdef AET2_ALIASDB_IS_PRESENT
    if (GLOBALS->adb_idx_last) {
        free_2(GLOBALS->adb_idx_last);
        GLOBALS->adb_idx_last = NULL;
    }
    if (GLOBALS->adb_idx_first) {
        free_2(GLOBALS->adb_idx_first);
        GLOBALS->adb_idx_first = NULL;
    }
    if (missing) {
        free_2(missing);
        missing = NULL;
    ...
    if (GLOBALS->ae2_time_xlate) /* GLOBALS->ae2_time_xlate is currently unused, but could be again
                                    in the future */
    {
        GLOBALS->min_time = GLOBALS->ae2_time_xlate[0];
        GLOBALS->max_time = GLOBALS->ae2_time_xlate[last_cycle - first_cycle];
    } else {
        GLOBALS->min_time = first_cycle;
        GLOBALS->max_time = last_cycle;
    }
    ...
    if (skip_start || skip_end) {
        ...
        if (GLOBALS->ae2_time_xlate) /* GLOBALS->ae2_time_xlate is currently unused, but could be
                                        again in the future */
        {
    ...
    fprintf(stderr,
            AET2_RDLOAD "[%" GW_TIME_FORMAT "] start time.\n" AET2_RDLOAD "[%" GW_TIME_FORMAT
                        "] end time.\n",
            GLOBALS->min_time,
            GLOBALS->max_time);
...
int ae2_iterator(uint64_t start_cycle, uint64_t end_cycle)
{
    ...
    for (j = 0; j < GLOBALS->ae2_num_sections; j++) {
        ...
        if ((ecyc < cyc) || (ecyc == ~GW_UTIME_CONSTANT(0)))
            continue;
        ...
        for (i = 0; i < (unsigned int)GLOBALS->numfacs; i++) {
            if (aet2_rd_get_fac_process_mask(i)) {
                ...
                if (nr < 2) {
                    ...
                    unsigned long sf =
                        ae2_read_symbol_sparse_flag(GLOBALS->ae2, GLOBALS->ae2_fr[i].s);
                    if (sf) {
                        unsigned int rows =
                            ae2_read_num_sparse_rows(GLOBALS->ae2, GLOBALS->ae2_fr[i].s, cyc);
                        if (rows) {
                            for (r = 1; r < rows + 1; r++) {
                                nptr np;
                                uint64_t row = ae2_read_ith_sparse_row(GLOBALS->ae2,
                                                                       GLOBALS->ae2_fr[i].s,
                                                                       cyc,
                                                                       r);
                        ...
                        unsigned int rows =
                            ae2_read_symbol_rows_2(GLOBALS->ae2, GLOBALS->ae2_fr[i].s);
                        if (rows) {
                            for (r = 0; r < rows; r++) {
                                nptr np;
                                uint64_t row = r;
            ...
            if (nr < 2) {
                ...
                if (sf) {
                    ...
                    for (r = 1; r < rows + 1; r++) {
                        /* nptr np; */
                        uint64_t row =
                            ae2_read_ith_sparse_row(GLOBALS->ae2, GLOBALS->ae2_fr[i].s, cyc, r);

                        GLOBALS->ae2_fr[i].row = row;
                        /* np = GLOBALS->ae2_lx2_table[i][row].np; */
                        ncyc = ae2_read_next_value_2(GLOBALS->ae2, GLOBALS->ae2_fr + i, cyc, buf);

                        if ((ncyc > cyc) && (ncyc < mxcyc))
                            mxcyc = ncyc;
                    }
            ...
            if (t) {
                while (t) {
                    ...
                    if (nr < 2) {
                        ...
                        ncyc = ae2_read_next_value_2(GLOBALS->ae2,
                                                     GLOBALS->ae2_fr + i,
                                                     step_cyc,
                                                     np->mv.value);
                    } else {
                        unsigned long sf =
                            ae2_read_symbol_sparse_flag(GLOBALS->ae2, GLOBALS->ae2_fr[i].s);
                        if (sf) {
                            unsigned int rows = ae2_read_num_sparse_rows(GLOBALS->ae2,
                                                                         GLOBALS->ae2_fr[i].s,
                                                                         step_cyc);
                            uint64_t mxcyc = end_cycle + 1;
    ...
    for (txidx = 0; txidx < GLOBALS->numfacs; txidx++) {
        if (aet2_rd_get_fac_process_mask(txidx)) {
            ...
            for (r = 0; r < nr; r++) {
                ...
                np->head.next = htemp;
                np->numhist =
                    GLOBALS->ae2_lx2_table[txidx][r].numtrans + 2 /*endcap*/ + 1 /*frontcap*/;

                np->curr = histent_tail;
                np->mv.mvlfac =
                    NULL; /* it's imported and cached so we can forget it's an mvlfac now */
            }
