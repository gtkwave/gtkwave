        ...
        while (t2) {
            {
                ...
                gtk_list_store_set(GTK_LIST_STORE(GLOBALS->sig_store_hiersearch),
                                   &iter,
                                   NAME_COLUMN,
                                   tmp,
                                   PTR_COLUMN,
...
static void enter_callback_e(GtkWidget *widget, GtkWidget *nothing)
{
    ...
    if (!(len = strlen(entry_text)))
        ...
        strcpy((GLOBALS->entrybox_text_local_hiersearch_c_1 = (char *)malloc_2(len + 1)),
               entry_text);
...
static void entrybox_local(char *title, int width, char *default_text, int maxch, GCallback func)
{
    ...
    GLOBALS->window1_hiersearch_c_1 =
        gtk_window_new(GLOBALS->disable_window_manager ? GTK_WINDOW_POPUP : GTK_WINDOW_TOPLEVEL);
    install_focus_cb(GLOBALS->window1_hiersearch_c_1,
                     ((char *)&GLOBALS->window1_hiersearch_c_1) - ((char *)GLOBALS));
    ...
    gtkwave_signal_connect(GLOBALS->window1_hiersearch_c_1,
                           "delete_event",
    ...
    gtkwave_signal_connect(GLOBALS->entry_hiersearch_c_2,
                           "activate",
                           G_CALLBACK(enter_callback_e),
                           GLOBALS->entry_hiersearch_c_2);
    gtk_entry_set_text(GTK_ENTRY(GLOBALS->entry_hiersearch_c_2), default_text);
    gtk_editable_select_region(GTK_EDITABLE(GLOBALS->entry_hiersearch_c_2),
                               0,
                               gtk_entry_get_text_length(GTK_ENTRY(GLOBALS->entry_hiersearch_c_2)));
    ...
    while (t) {
        if (t->child) {
            if (t->child->child) {
                ...
                add_vector_range(NULL, fetchlow(t)->t_which, fetchhigh(t)->t_which, direction);
            }
        }
        if (level) {
            t = t->next;
...
static void ok_callback(GtkWidget *widget, GtkWidget *nothing)
{
    ...
    if ((!GLOBALS->h_selectedtree_hiersearch_c_1) ||
        (!GLOBALS->h_selectedtree_hiersearch_c_1->child))
        return;
...
void hier_searchbox(char *title, GCallback func)
{
    ...
    /* create a new modal window */
    GLOBALS->window_hiersearch_c_3 =
        gtk_window_new(GLOBALS->disable_window_manager ? GTK_WINDOW_POPUP : GTK_WINDOW_TOPLEVEL);
    install_focus_cb(GLOBALS->window_hiersearch_c_3,
                     ((char *)&GLOBALS->window_hiersearch_c_3) - ((char *)GLOBALS));

    gtk_window_set_title(GTK_WINDOW(GLOBALS->window_hiersearch_c_3), title);
    gtkwave_signal_connect(GLOBALS->window_hiersearch_c_3,
                           "delete_event",
                           (GCallback)destroy_callback,
                           NULL);
    gtk_container_set_border_width(GTK_CONTAINER(GLOBALS->window_hiersearch_c_3), 12);

    GtkWidget *main_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    gtk_container_add(GTK_CONTAINER(GLOBALS->window_hiersearch_c_3), main_vbox);

    GtkWidget *label = gtk_label_new("Signal Hierarchy");
	...
	gtk_widget_set_can_focus(GLOBALS->entry_main_hiersearch_c_1, FALSE);
    gtk_tooltips_set_tip_2(GLOBALS->entry_main_hiersearch_c_1,
                           ...
                           "go up a level.");
    gtk_box_pack_start(GTK_BOX(main_vbox), GLOBALS->entry_main_hiersearch_c_1, FALSE, FALSE, 0);

    GLOBALS->sig_store_hiersearch = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER);
    GtkWidget *sig_view =
        gtk_tree_view_new_with_model(GTK_TREE_MODEL(GLOBALS->sig_store_hiersearch));
    ...
    gtk_tree_selection_set_select_function(GLOBALS->sig_selection_hiersearch,
	...
	gtk_widget_set_margin_top(scrolled_win, 12);
	gtk_widget_set_margin_bottom(scrolled_win, 12);
    ...
    gtk_box_pack_start(GTK_BOX(main_vbox), scrolled_win, FALSE, FALSE, 0);

    GtkWidget *button_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
	gtk_box_set_homogeneous(GTK_BOX(button_box), TRUE);
    gtk_box_pack_start(GTK_BOX(main_vbox), button_box, FALSE, FALSE, 0);

    GtkWidget *button = gtk_button_new_with_label("Append");
    gtkwave_signal_connect_object(button,
                                  "clicked",
                                  G_CALLBACK(ok_callback),
                                  GLOBALS->window_hiersearch_c_3);
    gtk_tooltips_set_tip_2(button,
    ...
    if (GLOBALS->vcd_explicit_zero_subscripts >= 0) {
        button = gtk_button_new_with_label("Bundle Up");
        gtkwave_signal_connect_object(button,
                                      "clicked",
                                      G_CALLBACK(bundle_callback_up),
                                      GLOBALS->window_hiersearch_c_3);
        gtk_tooltips_set_tip_2(button,
                               "Bundle children into a single bit vector with the topmost signal "
                               "as the LSB and the lowest as the MSB.");
        gtk_box_pack_start(GTK_BOX(button_box), button, TRUE, TRUE, 0);

        button = gtk_button_new_with_label("Bundle Down");
        gtkwave_signal_connect_object(button,
                                      "clicked",
        ...
        gtk_tooltips_set_tip_2(button,
                               "Bundle children into a single bit vector with the topmost signal "
                               "as the MSB and the lowest as the LSB.");
        gtk_box_pack_start(GTK_BOX(button_box), button, TRUE, TRUE, 0);
    }
    ...
    gtkwave_signal_connect_object(button,
                                  ...
                                  GLOBALS->window_hiersearch_c_3);
    gtk_tooltips_set_tip_2(button, "Do nothing and return to the main window.");
    gtk_box_pack_start(GTK_BOX(button_box), button, TRUE, TRUE, 0);

    gtk_widget_show_all(GLOBALS->window_hiersearch_c_3);
