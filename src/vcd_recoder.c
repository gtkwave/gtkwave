...
static void vlist_packer_emit_utt(struct vlist_packer_t **vl, GwUTime v)
{
    GwUTime nxt;
...
static void write_fastload_time_section(void)
{
    ...
    if (GLOBALS->blackout_regions) {
        ...
        vlist_packer_emit_uv32(
            (struct vlist_packer_t **)(void *)&GLOBALS->time_vlist_vcd_recoder_write,
            bcnt);

        bt = GLOBALS->blackout_regions;
        while (bt) {
            vlist_packer_emit_utt(
                (struct vlist_packer_t **)(void *)&GLOBALS->time_vlist_vcd_recoder_write,
                bt->bstart);
            vlist_packer_emit_utt(
                (struct vlist_packer_t **)(void *)&GLOBALS->time_vlist_vcd_recoder_write,
...
#endif
{
    ...
    number of variable-length integer values in the time table
    (GLOBALS->time_vlist_count_vcd_recoder_c_1) GLOBALS->time_vlist_vcd_recoder_write value deltas
    from GLOBALS->time_vlist_vcd_recoder_write and on though stepping through list generated by
    vlist_emit_finalize()
    */
                    ...
                    if (GLOBALS->yytext_vcd_recoder_c_3[0] == '#') {
                        ...
                        if (GLOBALS->start_time_vcd_recoder_c_3 < 0) {
                            ...
                            if (GLOBALS->time_vlist_vcd_recoder_write) {
                                vlist_packer_emit_utt((struct vlist_packer_t **)(void *)&GLOBALS
                                                          ->time_vlist_vcd_recoder_write,
                                                      tim);
                            }
                        } else {
                            /* backtracking fix */
            ...
            if (GLOBALS->vcd_warning_filesize)
                if (GLOBALS->vcd_fsiz_vcd_recoder_c_2 >
                    ...
                    if (!GLOBALS->vlist_prepack) {
                        ...
                        fprintf(stderr,
                                "VCDLOAD | File size is %d MB, using vlist prepacking%s.\n\n",
                                (int)(GLOBALS->vcd_fsiz_vcd_recoder_c_2 / (1024 * 1024)),
