...
const char *extload_loader_fail_msg =
...
static int last_modification_check(void)
{
    ...
    if (GLOBALS->extload_lastmod) {
        if (GLOBALS->extload_already_errored) {
            ...
            return (0);
        } else if (GLOBALS->extload_lastmod != buf.st_mtime) {
...
#ifdef WAVE_FSDB_READER_IS_PRESENT
static char *get_varname(char *sbuff,
                         unsigned char *vtp,
                         unsigned char *vdp,
                         int i,
                         int *patched_len)
...
#endif
    {
        ...
        if (rc) {
            if (isspace(rc[0])) {
                ...
                } else if ((snp = strstr(rc + 1, "Struct End"))) {
                    sprintf(rc, "Upscope:\n");
                }
            }
        } else {
        ...
        if ((rc[0] == 'V') && (i >= 0)) {
...
#endif
            {
                ...
                *typ_dst = 0;
                ...
                while (*pnt) {
                    if ((pnt[0] == 'l') && (pnt[1] == ':')) {
                        ...
                        if (pnt[0] == '[') {
                            ...
                            if (pnt[1] == '[')
                                esc = pnt; /* pretend we're escaped to handle 2d */
                        } else if (pnt[0] == ':') {
                            if (state) {
                                colon = pnt;
                            }
                ...
                if (last_l) {
                    ...
                    } else if (!strcmp("vcd_integer", typ)) {
                        ...
                        if (esc && lb && rb) {
                            ...
                            if (colon) {
                                ...
                                GLOBALS->extload_node_block[i].lsi =
                                    GLOBALS->extload_node_block[i].msi;
                            }

                            len_parse = (GLOBALS->extload_node_block[i].msi >
                                         GLOBALS->extload_node_block[i].lsi)
                                            ? (GLOBALS->extload_node_block[i].msi -
                                               GLOBALS->extload_node_block[i].lsi + 1)
                                            : (GLOBALS->extload_node_block[i].lsi -
                                               GLOBALS->extload_node_block[i].msi + 1);

                            if ((GLOBALS->mvlfacs_vzt_c_3[i].len > len_parse) &&
                                !(GLOBALS->mvlfacs_vzt_c_3[i].len %
                                  len_parse)) /* check if 2d array */
                            {
                    ...
                    if (vt_len > 4) {
                        if (!strncmp(vtyp_nam, "vcd_", 4)) {
                            ...
                            if (!strcmp(vtyp_nam, "stream")) {
                                GLOBALS->extload_idcodes[i] =
                                    0; /* kill being able to read stream variables [transactions]
                                          for now */
                            }
                ...
                if (*pnt) {
                    while (*pnt) {
                        ...
                        if (isspace(*pnt))
                            break;
                        if ((*pnt == '[') && (pnt == strrchr(pnt, '[')) &&
                            (GLOBALS->mvlfacs_vzt_c_3[i].flags !=
                             VZT_RD_SYM_F_DOUBLE)) /* fix for arrays */
                        {
                            ...
                            if (lastch == ']') /* fix for NC verilog arrays */
                            {
                                ...
                                rng = GLOBALS->extload_node_block[i].msi -
                                      GLOBALS->extload_node_block[i].lsi;
                                if (!rng) {
                                    break;
                                }
                        ...
                        if (*pnt == '\\') /* this is not strictly correct, but fixes generic ranges
                                             from icarus */
                        {
                            pnt++;
                            continue;
...
#endif
            {
                ...
                GLOBALS->fst_scope_name =
                    fstReaderPushScope(GLOBALS->extload_xc, cname, GLOBALS->mod_tree_parent);
                if (!strcmp(ctype, "NULL")) {
                    ctype[0] = 0;
                }
...
#endif
{
    ...
    if (i < 0) {
#ifdef WAVE_FSDB_READER_IS_PRESENT
        fnam = get_varname(s_gv,
                           &GLOBALS->extload_vt_prev,
                           &GLOBALS->extload_vd_prev,
                           0,
                           &patched_len);
        flen = strlen(fnam);

        if (GLOBALS->extload_hlen) {
            GLOBALS->extload_namecache[0 & F_NAME_MODULUS] =
                malloc_2(GLOBALS->extload_namecache_max[0 & F_NAME_MODULUS] =
                             GLOBALS->extload_hlen + 1 + flen + 1);
            memcpy(GLOBALS->extload_namecache[0 & F_NAME_MODULUS],
                   GLOBALS->fst_scope_name,
                   GLOBALS->extload_hlen);
            *(GLOBALS->extload_namecache[0 & F_NAME_MODULUS] + GLOBALS->extload_hlen) = '.';
            strcpy(GLOBALS->extload_namecache[0 & F_NAME_MODULUS] + GLOBALS->extload_hlen + 1,
                   fnam);
            ...
            GLOBALS->extload_namecache[0 & F_NAME_MODULUS] =
                malloc_2(GLOBALS->extload_namecache_max[0 & F_NAME_MODULUS] = flen + 1);
        ...
        if (i != (GLOBALS->numfacs - 1)) {
            ...
            if (GLOBALS->extload_hlen) {
                if (GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] <
                    ...
                    GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS] =
                        malloc_2(GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] =
                                     GLOBALS->extload_hlen + 1 + flen + 1);
                }
                ...
                *(GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS] + GLOBALS->extload_hlen) =
                    '.';
                strcpy(GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS] +
                           GLOBALS->extload_hlen + 1,
                       fnam);
                ...
                if (GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] < (flen + 1)) {
                    if (GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS])
                        free_2(GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS]);
                    GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS] = malloc_2(
                        GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] = flen + 1);
                }
            ...
            if (GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] < (flen + 1)) {
                GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS] =
                    malloc_2(GLOBALS->extload_namecache_max[(i + 1) & F_NAME_MODULUS] = flen + 1);
            }
            strcpy(GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS], fnam);
            GLOBALS->extload_namecache_lens[(i + 1) & F_NAME_MODULUS] = flen;
            GLOBALS->extload_namecache_patched[(i + 1) & F_NAME_MODULUS] = patched_len;
        ...
        if ((f->len > 1) &&
            ...
            {
                GLOBALS->extload_node_block[i].msi =
                    GLOBALS->extload_namecache_patched[i & F_NAME_MODULUS] - 1;
                GLOBALS->extload_node_block[i].lsi = 0;
            }
            ...
            GLOBALS->extload_prevsymroot = GLOBALS->extload_prevsym = NULL;
                    ...
                    ((i != GLOBALS->numfacs - 1) &&
                     (GLOBALS->extload_namecache_lens[i & F_NAME_MODULUS] ==
                      GLOBALS->extload_namecache_lens[(i + 1) & F_NAME_MODULUS]) &&
                     (!memrevcmp(GLOBALS->extload_namecache_lens[i & F_NAME_MODULUS],
                                 GLOBALS->extload_namecache[i & F_NAME_MODULUS],
                                 GLOBALS->extload_namecache[(i + 1) & F_NAME_MODULUS])))) ||
                   (((i != 0) &&
                     (GLOBALS->extload_namecache_lens[i & F_NAME_MODULUS] ==
            ...
            s = &GLOBALS->extload_sym_block[i];
            symadd_name_exists_sym_exists(s, str, 0);
            if ((GLOBALS->extload_prevsym) && (i > 0) &&
                (GLOBALS->extload_namecache_lens[i & F_NAME_MODULUS] ==
                 GLOBALS->extload_namecache_lens[(i - 1) & F_NAME_MODULUS]) &&
                (!memrevcmp(
                    GLOBALS->extload_namecache_lens[i & F_NAME_MODULUS],
...
static void extload_hiertree_callback(void *pnt)
{
    ...
    if ((GLOBALS->extload_curr_tree < GLOBALS->extload_max_tree) || (!GLOBALS->extload_max_tree)) {
        switch (s[0]) {
            case 'S':
            case 'U':
                get_varname(s, NULL, NULL, -1, &patched_len);
                GLOBALS->extload_hlen =
                    GLOBALS->fst_scope_name ? strlen(GLOBALS->fst_scope_name) : 0;
                break;

            case 'V':
                process_extload_variable(s);
                break;

            case 'E': {
                GLOBALS->extload_curr_tree++;
                fprintf(stderr,
                        EXTLOAD "End tree #%d: %d vs %d symbols\n",
                        GLOBALS->extload_curr_tree,
                        GLOBALS->extload_i + 1,
                        GLOBALS->numfacs);
                if ((GLOBALS->extload_curr_tree == GLOBALS->extload_max_tree) &&
                    (GLOBALS->extload_max_tree)) {
                    if (GLOBALS->numfacs > (GLOBALS->extload_i + 1)) {
                        fprintf(stderr,
                                EXTLOAD "Max tree count of %d processed, freeing extra memory.\n",
                                GLOBALS->extload_max_tree);
...
static char *get_varname2(struct fstHier *fh,
                          unsigned char *vtp,
                          unsigned char *vdp,
                          int i,
                          int *patched_len)
{
    ...
    *patched_len = 0; /* zero says is ok, otherwise size overrides msi/lsi */

    {
        if ((fh->htyp == FST_HT_VAR) && (i >= 0)) {
            {
                ...
                pnt = (fh->u.var.name[fh->u.var.name_length - 1] == ']')
                          ? fh->u.var.name
                          : &fh->u.var.name[fh->u.var.name_length];
                while (*pnt) {
                    if (pnt[0] == '\\') {
                        /* esc = pnt; */
                    } else {
...

                {
                    ...
                    if (fh->u.var.typ == FST_VT_VCD_REAL) {
                        ...
                        GLOBALS->extload_node_block[i].lsi = 0;
                        GLOBALS->mvlfacs_vzt_c_3[i].len = 64;
                    } else if (fh->u.var.typ == FST_VT_VCD_INTEGER) {
                        GLOBALS->mvlfacs_vzt_c_3[i].flags = VZT_RD_SYM_F_INTEGER;
                        GLOBALS->extload_node_block[i].msi = 0;
                        GLOBALS->extload_node_block[i].lsi = 0;
                        GLOBALS->mvlfacs_vzt_c_3[i].len = 32;
                ...
                if (*pnt) {
                    while (*pnt) {
                        ...
                        if ((*pnt == '[') && (pnt == strrchr(pnt, '[')) &&
                            (fh->u.var.typ != FST_VT_VCD_REAL)) /* fix for arrays */
                        {
                            /* now to fix possible generate... */
                            const char *pnt2 = pnt;
                            char lastch = *pnt2;
        ...
        } else if (fh->htyp == FST_HT_SCOPE) {
            {
                unsigned char ttype;

                GLOBALS->fst_scope_name = fstReaderPushScope(GLOBALS->extload_xc,
                                                             fh->u.scope.name,
                                                             GLOBALS->mod_tree_parent);
                ...
                allocate_and_decorate_module_tree_node(ttype,
                                                       fh->u.scope.name,
                                                       fh->u.scope.component,
                                                       fh->u.scope.name_length,
                                                       fh->u.scope.component_length,
                                                       0,
                                                       0);
            }
        } else if (fh->htyp == FST_HT_UPSCOPE) {
            GLOBALS->mod_tree_parent = fstReaderGetCurrentScopeUserInfo(GLOBALS->extload_xc);
            GLOBALS->fst_scope_name = fstReaderPopScope(GLOBALS->extload_xc);
        }
    }
...
static void process_extload_variable2(struct fstHier *s_gv)
{
    ...
    if (i < 0) {
        ...
        if ((f->len > 1) &&
            ...
            if (GLOBALS->fast_tree_sort) {
                fsdb_append_graft_chain(strlen(fnam_prev),
                                        fnam_prev,
                                        i,
                                        GLOBALS->extload_npar[i & F_NAME_MODULUS]);
                if (fnam)
                    strcpy(fnam_prev, fnam);
            }
        }
...
static void extload_hiertree_callback2(void *pnt)
{
    ...
    if ((GLOBALS->extload_curr_tree < GLOBALS->extload_max_tree) || (!GLOBALS->extload_max_tree)) {
        switch (s->htyp) {
            case FST_HT_SCOPE:
            case FST_HT_UPSCOPE:
                get_varname2(s, NULL, NULL, -1, &patched_len);
                GLOBALS->extload_hlen =
                    GLOBALS->fst_scope_name ? strlen(GLOBALS->fst_scope_name) : 0;
                break;
 ...
 * mainline
 */
static GwTime extload_main_2(char *fname, char *skip_start, char *skip_end)
{
    int max_idcode;
#ifndef WAVE_FSDB_READER_IS_PRESENT
    char sbuff[65537];
    ...
    if (GLOBALS->extload_ffr_ctx) {
        ...
        if (rv) {
            ...
            if (GLOBALS->max_time == GW_TIME_CONSTANT(0)) {
                GLOBALS->max_time = GW_TIME_CONSTANT(1);
            }
            success_count++;
        }
            ...
            case 'm':
                if (!strncmp("minimum xtag", rc, 12)) {
                    ...
                    if (pnt) {
                        ...
                        GLOBALS->min_time = (GwTime)((((GwUTime)hi) << 32) + ((GwUTime)lo));
                    ...
                    if (pnt) {
                        ...
                        pnt++;
                        sscanf(pnt, "%u %u", &hi, &lo);
                        GLOBALS->max_time = (GwTime)((((GwUTime)hi) << 32) + ((GwUTime)lo));
                        if (GLOBALS->max_time == GW_TIME_CONSTANT(0)) {
                            GLOBALS->max_time = GW_TIME_CONSTANT(1);
                        }
    ...
    free_2(GLOBALS->extload_namecache_max);
    GLOBALS->extload_namecache_max = NULL;
    free_2(GLOBALS->extload_namecache_lens);
    GLOBALS->extload_namecache_lens = NULL;
    free_2(GLOBALS->extload_namecache_patched);
    GLOBALS->extload_namecache_patched = NULL;
#ifdef WAVE_USE_FSDB_FST_BRIDGE
    free_2(GLOBALS->extload_npar);
    GLOBALS->extload_npar = NULL;
#endif
    ...
    while (get_varname(&GLOBALS->extload_vt_prev, NULL, -1, &patched_len))
        ; /* read through end to process all upscopes */
...
GwTime extload_main(char *fname, char *skip_start, char *skip_end)
{
    GwTime tt = extload_main_2(fname, skip_start, skip_end);
...
void import_extload_trace(GwNode *np)
{
    ...
    } else /* "normal" VC data */
    {
        ...
        if (fsdbReaderHasIncoreVC(GLOBALS->extload_ffr_ctx, hdl)) {
            GwTime mxt_max = -2;
            GwTime mxt = (GwTime)fsdbReaderGetMinXTag(GLOBALS->extload_ffr_ctx, hdl);
            ...
            while (rc_xtag && (mxt >= mxt_max)) /* malformed traces sometimes backtrack time */
            {
                ...
                mxt_max = mxt;
                mxt = (GwTime)fsdbReaderGetXTag(GLOBALS->extload_ffr_ctx, hdl, &rc_xtag);
            }
        }
    ...
    if (last_modification_check()) /* place array height check here in an "&&" branch, sorry, arrays
                                      not supported */
    {
        char sbuff[65537];
        GwTime tim;
        ...
        for (;;) {
            ...
            if (isdigit(rc[0])) {
                ...
                if (rc) {
                    ...
                    tim = (GwTime)((((GwUTime)hi) << 32) + ((GwUTime)lo));
