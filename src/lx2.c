...
TimeType lx2_main(char *fname, char *skip_start, char *skip_end)
{
    ...
    for (i = 0; i < GLOBALS->numfacs; i++) {
        ...
        if ((f->len > 1) &&
            (!(f->flags & (LXT2_RD_SYM_F_INTEGER | LXT2_RD_SYM_F_DOUBLE | LXT2_RD_SYM_F_STRING)))) {
            int len = sprintf(buf,
                              "%s[%d:%d]",
            ...
            int gatecmp = (f->len == 1) &&
                          (!(f->flags & (LXT2_RD_SYM_F_INTEGER | LXT2_RD_SYM_F_DOUBLE |
                                         LXT2_RD_SYM_F_STRING))) &&
                          (node_block[i].msi != -1) && (node_block[i].lsi != -1);
            int revcmp = gatecmp && (i) &&
                         (!strcmp(f_name[(i)&F_NAME_MODULUS], f_name[(i - 1) & F_NAME_MODULUS]));
    ...
    GLOBALS->min_time = GLOBALS->first_cycle_lx2_c_1;
    GLOBALS->max_time = GLOBALS->last_cycle_lx2_c_1;
 ...
 */
static void lx2_callback(struct lxt2_rd_trace **lt,
                         lxtint64_t *tim,
                         lxtint32_t *facidx,
                         char **value)
{
...
void import_lx2_trace(nptr np)
{
    ...
    if (f->node_alias <=
        1) /* sorry, arrays not supported, but lx2 doesn't support them yet either */
    {
    ...
    if (!(f->flags & (LXT2_RD_SYM_F_DOUBLE | LXT2_RD_SYM_F_STRING))) {
        ...
        if (f->flags & LXT2_RD_SYM_F_STRING)
    ...
    for (txidx = 0; txidx < GLOBALS->numfacs; txidx++) {
        if (lxt2_rd_get_fac_process_mask(GLOBALS->lx2_lx2_c_1, txidx)) {
...

            {
                ...
                if (f->flags & (LXT2_RD_SYM_F_DOUBLE | LXT2_RD_SYM_F_STRING)) {
                    htemp2->v.h_vector =
                        strdup_2((f->flags & LXT2_RD_SYM_F_DOUBLE) ? "NaN" : "UNDEF");
            ...
            np->head.next = htemp;
            np->numhist =
                GLOBALS->lx2_table_lx2_c_1[txidx].numtrans + 2 /*endcap*/ + 1 /*frontcap*/;

            memset(GLOBALS->lx2_table_lx2_c_1 + txidx,
                   0,
