    ...
    if ((t = GLOBALS->named_markers[i]) != -1) {
        if ((t >= GLOBALS->tims.start) && (t <= GLOBALS->tims.last) && (t <= GLOBALS->tims.end)) {
            ...
            if ((xl >= 0) && (xl < GLOBALS->wavewidth)) {
                ...
                cairo_set_dash(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                               dashed1,
                               sizeof(dashed1) / sizeof(dashed1[0]),
                               0);
                ...
                if ((!GLOBALS->marker_names[i]) || (!GLOBALS->marker_names[i][0])) {
                    XXX_font_engine_draw_string(
                        GLOBALS->cr_wavepixmap_wavewindow_c_1,
                    ...
                    XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                                                GLOBALS->wavefont_smaller,
                                                &gc,
...
static void draw_named_markers(void)
{
    ...
    if (GLOBALS->named_marker_lock_idx >= 0) {
        render_individual_named_marker(GLOBALS->named_marker_lock_idx,
...
static void draw_marker(void)
{
    ...
    if (!gtk_widget_get_window(GLOBALS->wavearea))
        return;
...
#endif
    cairo_t *cr =
        XXX_gdk_cairo_create(XXX_GDK_DRAWABLE(gtk_widget_get_window(GLOBALS->wavearea)), &gdc);
    ...
    if (GLOBALS->m1x_wavewindow_c_1 >
        GLOBALS->m2x_wavewindow_c_1) /* ensure m1x <= m2x for partitioned refresh */
    {
        gint t;
    ...
    if (GLOBALS->m1x_wavewindow_c_1 == -1)
        GLOBALS->m1x_wavewindow_c_1 =
            GLOBALS->m2x_wavewindow_c_1; /* make both markers same if no ghost marker or v.v. */
    ...
    if (bt) {
        ...
        while (bt) {
            if ((bt->bend < lhs) || (bt->bstart > rhs)) {
                ...
                XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1,
...
static void service_hslider(GtkWidget *text, gpointer data)
{
    ...
    if ((GLOBALS->cr_wavepixmap_wavewindow_c_1) && (GLOBALS->wavewidth > 1)) {
        ...
        {
            XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                                   GLOBALS->rgb_gc.gc_back_wavewindow_c_1,
                                   TRUE,
                                   0,
                                   0,
                                   GLOBALS->wavewidth,
...
gint wavearea_configure_event(GtkWidget *widget, GdkEventConfigure *event)
{
    ...
    if (GLOBALS->cr_wavepixmap_wavewindow_c_1) {
        if ((GLOBALS->wavewidth != allocation.width) ||
            ...
            GLOBALS->surface_wavepixmap_wavewindow_c_1 =
                cairo_image_surface_create(CAIRO_FORMAT_RGB24,
                                           GLOBALS->wavewidth * scale_factor,
                                           allocation.height * scale_factor);
            GLOBALS->cr_wavepixmap_wavewindow_c_1 =
                cairo_create(GLOBALS->surface_wavepixmap_wavewindow_c_1);
...
GtkWidget *create_wavewindow(void)
{
    ...
    g_signal_connect(GLOBALS->wavearea,
                     "configure_event",
                     G_CALLBACK(wavearea_configure_event_local),
                     NULL);
...
static void renderhash(int x, TimeType tim)
{
    ...
    cairo_set_source_rgba(GLOBALS->cr_wavepixmap_wavewindow_c_1,
    ...
    cairo_move_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                  x + WAVE_CAIRO_050_OFFSET,
                  WAVE_CAIRO_050_OFFSET);
    cairo_line_to(
        GLOBALS->cr_wavepixmap_wavewindow_c_1,
        x + WAVE_CAIRO_050_OFFSET,
    ...
    cairo_move_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                  x + WAVE_CAIRO_050_OFFSET,
                  GLOBALS->wavecrosspiece + WAVE_CAIRO_050_OFFSET);
    cairo_line_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                  rhs + WAVE_CAIRO_050_OFFSET,
                  GLOBALS->wavecrosspiece + WAVE_CAIRO_050_OFFSET);
    cairo_stroke(GLOBALS->cr_wavepixmap_wavewindow_c_1);
    ...
    while ((hashoffset < GLOBALS->pixelsperframe) && (x <= rhs) && (iter < 9)) {
        ...
        cairo_line_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                      x + WAVE_CAIRO_050_OFFSET,
                      fhminus2 + WAVE_CAIRO_050_OFFSET);
        cairo_stroke(GLOBALS->cr_wavepixmap_wavewindow_c_1);
                ...
                for (; pos < GLOBALS->strace_ctx->timearray_size; t++, pos++) {
                    ...
                    if (tm >= GLOBALS->tims.start) {
                        if (tm <= GLOBALS->tims.end) {
                            ...
                            cairo_move_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                                          x + WAVE_CAIRO_050_OFFSET,
                                          y + WAVE_CAIRO_050_OFFSET);
                            cairo_line_to(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                                          x + WAVE_CAIRO_050_OFFSET,
                                          GLOBALS->waveheight + WAVE_CAIRO_050_OFFSET);
                            cairo_stroke(GLOBALS->cr_wavepixmap_wavewindow_c_1);
                        } else {
                            break;
                        }
    ...
    for (;;) {
        ...
        if ((x - lenhalf >= lastx) || (GLOBALS->pixelsperframe >= 200)) {
            XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                                        ...
                                        x - lenhalf + WAVE_CAIRO_050_OFFSET,
                                        GLOBALS->wavefont->ascent - 1 + WAVE_CAIRO_050_OFFSET,
                                        timebuff);
            lastx = x + lenhalf;
        }
                            ...
                            if ((width = _x1 - _x0_new) > GLOBALS->vector_padding) {
                                if ((_x1 >= GLOBALS->wavewidth) ||
                                    ...
                                    XXX_font_engine_draw_string(
                                        ...
                                        _x0 + 2 + WAVE_CAIRO_050_OFFSET,
                                        ytext + WAVE_CAIRO_050_OFFSET,
                                        identifier_str);
                                }
                            }
                        }
