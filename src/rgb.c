...
#define WAVE_RGB_COLOR(_name, r, g, b) \
    { \
        _name, (r << 16) | (g << 8) | b \
    }
...
int get_rgb_from_name(const char *str)
{
    ...
    if ((match = (struct wave_rgb_color *)bsearch((void *)str,
                                                  (void *)colors,
                                                  C_ARRAY_SIZE,
                                                  sizeof(struct wave_rgb_color),
                                                  compar))) {
        return (match->rgb);
    } else {
...
wave_rgb_t XXX_get_gc_from_name(const char *str)
{
    ...
    if ((match = (struct wave_rgb_color *)bsearch((void *)str,
        ...
        red = (match->rgb >> 16) & 0x000000ff;
        green = (match->rgb >> 8) & 0x000000ff;
        blue = (match->rgb) & 0x000000ff;
